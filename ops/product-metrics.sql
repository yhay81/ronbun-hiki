SELECT
  ...
  COUNT(DISTINCT CASE WHEN is_qa = 0 AND event_name = 'doi_opened' THEN session_hash END) AS doi_openers,
  ...
  COUNT(DISTINCT CASE WHEN is_qa = 0 AND event_name = 'correction_seen' THEN session_hash END) AS correction_viewers,
  ...
  COUNT(DISTINCT CASE WHEN is_qa = 0 AND event_name = 'doi_opened' AND created_at >= unixepoch() - 7 * 86400 THEN session_hash END) AS doi_openers_7d,
